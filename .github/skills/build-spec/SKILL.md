...
2. Run discovery in batches, not as a giant questionnaire.
3. Lock product truth in `SPEC.md` first.
4. If infrastructure or runtime topology is already known, capture the major
...
5. Translate that into routes and states in `UI_PLAN.md`.
6. Lock server and integration behavior in `CONTRACT.md`.
7. End each pass with three buckets:
   - Locked decisions
...
- Ask at most 5 to 7 questions at a time unless the user explicitly wants a
  full checklist.
